-- =====================================================
-- NEURATIVO DATABASE SETUP - FINAL VERSION
...
-- Drop existing policies for users table (only if table exists)
DO $$
BEGIN
...
-- Drop existing policies for user_subscriptions table (only if table exists)
DO $$
BEGIN
...
-- Drop existing policies for usage_stats table (only if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'usage_stats' AND table_schema = 'public') THEN
...
-- Drop existing policies for ai_usage_logs table (only if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'ai_usage_logs' AND table_schema = 'public') THEN
...
BEGIN
    ...
    DROP TRIGGER IF EXISTS reset_daily_usage_trigger ON public.usage_stats;
    DROP TRIGGER IF EXISTS reset_monthly_usage_trigger ON public.usage_stats;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'All errors handled gracefully';
END $$; 