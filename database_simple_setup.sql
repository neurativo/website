-- =====================================================
-- SIMPLE DATABASE SETUP FOR NEURATIVO
-- Just creates everything fresh - no drops, no checks
-- =====================================================
...
CREATE POLICY IF NOT EXISTS "Users can view own profile data" ON public.user_profiles
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own profile data" ON public.user_profiles
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own profile data" ON public.user_profiles
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Quizzes policies
CREATE POLICY IF NOT EXISTS "Anyone can view public quizzes" ON public.quizzes
...
CREATE POLICY IF NOT EXISTS "Anyone can view questions for public quizzes" ON public.quiz_questions
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can manage questions for own quizzes" ON public.quiz_questions
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.quizzes 
            WHERE id = quiz_id 
...
CREATE POLICY IF NOT EXISTS "Users can view own attempts" ON public.quiz_attempts
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create own attempts" ON public.quiz_attempts
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own attempts" ON public.quiz_attempts
    FOR UPDATE USING (auth.uid() = user_id);

-- User game stats policies
CREATE POLICY IF NOT EXISTS "Users can view own stats" ON public.user_game_stats
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own stats" ON public.user_game_stats
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own stats" ON public.user_game_stats
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Learning paths policies
CREATE POLICY IF NOT EXISTS "Users can view own learning paths" ON public.learning_paths
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can manage own learning paths" ON public.learning_paths
    FOR ALL USING (auth.uid() = user_id);

-- AI usage logs policies
CREATE POLICY IF NOT EXISTS "Users can view own AI usage" ON public.ai_usage_logs
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create own AI usage logs" ON public.ai_usage_logs
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Usage logs policies
CREATE POLICY IF NOT EXISTS "Users can view own usage" ON public.usage_logs
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create own usage logs" ON public.usage_logs
    FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Users can create own analytics" ON public.user_analytics
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- System notifications policies
CREATE POLICY IF NOT EXISTS "Users can view own notifications" ON public.system_notifications
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own notifications" ON public.system_notifications
    FOR UPDATE USING (auth.uid() = user_id);
...
CREATE TRIGGER IF NOT EXISTS update_user_profiles_updated_at BEFORE UPDATE ON public.user_profiles
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_quizzes_updated_at BEFORE UPDATE ON public.quizzes
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_user_game_stats_updated_at BEFORE UPDATE ON public.user_game_stats
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_learning_paths_updated_at BEFORE UPDATE ON public.learning_paths
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER IF NOT EXISTS on_quiz_question_change
...
BEGIN
    ...
    RAISE NOTICE 'All policies created with IF NOT EXISTS';
    RAISE NOTICE 'All triggers created with IF NOT EXISTS';
